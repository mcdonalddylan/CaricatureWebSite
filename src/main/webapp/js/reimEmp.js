...
async function toggleView()
{
    ...
    if(isApply === false && isView === false)
    {
        ...
        function LoadTable()
        {
        asyncFetch("http://3.135.241.211:8081/Caricature/userReim.json", function(reimbursements)
        {
            ...
            if(reimbursements[0].id != 0)
            {
                ...
                {
                    const fetched = await fetch("http://3.135.241.211:8081/Caricature/remReim.json?id=" + reim.id,
                    {method: 'post'});
